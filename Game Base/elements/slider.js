class slider extends element {
    ...
    process () {
        if(this.clicked) {
            ...
            if(isKeyDown('n')) {
                this.slidePos = Math.max(Math.min(invLerp(mouse.x, this.x-this.width/2+slideRadius, this.x+this.width/2-slideRadius), 1), 0);
            } else {
                this.slidePos = Math.round(
                Math.max(Math.min(invLerp(mouse.x, this.x-this.width/2+slideRadius, this.x+this.width/2-slideRadius), 1), 0)*
    ...
    renderBg() {
        ...
        if(this.clicked || (this.isMouseOver() && actionType == 'none')) {
            ...
            ctx.textAlign = 'left';
            ctx.fillText(Math.round(this.getSlideValue()), mouse.x+6, mouse.y+4);
        };
    };
};