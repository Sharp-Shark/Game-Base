...
class object {
    ...
    updateClosestObjectToMouse () {
        ...
        let d = distance([this.x, this.y], [worldMouse.x, worldMouse.y]);
        if((d < closestObjectToMouseDistance || closestObjectToMouseDistance == -1) && d <= this.size + 5/cam.zoom) {
            closestObjectToMouseDistance = distance([this.x, this.y], [worldMouse.x, worldMouse.y]);
    ...
    render () {
        circle(xToCam(this.x), yToCam(this.y), this.size*cam.zoom, 'rgb(175, 175, 175, 0.8)');
        circle(xToCam(this.x), yToCam(this.y), Math.max(this.size-10, this.size*0.8)*cam.zoom, 'rgb(155, 155, 155, 0.5)');
    };
};